        ...
        <div className="space-y-4">
          <div><label className="block text-sm text-[#888] mb-2">Channel</label>
            <div className="grid grid-cols-2 gap-3">
              {[{ type: "telegram", icon: "💬", label: "Telegram" }, { type: "discord", icon: "🎮", label: "Discord" }].map(ch => (
                <button key={ch.type} onClick={() => setChannelType(ch.type)} className={`p-4 rounded-lg border text-left transition ${channelType === ch.type ? "border-indigo-500 bg-indigo-500/10" : "border-[#1e1e1e] bg-black/30 hover:border-white/20"}`}>
                  <span className="text-2xl">{ch.icon}</span><div className="mt-1 font-medium text-sm">{ch.label}</div>
                </button>
              ))}
            </div>
          </div>
          <div><label className="block text-sm text-[#888] mb-2">{channelType === "telegram" ? "Telegram Bot Token" : "Discord Bot Token"}</label>
            ...
            <p className="text-xs text-[#888] mt-2">{channelType === "telegram" ? "Get this from @BotFather on Telegram." : "Create a bot at discord.com/developers/applications."}</p>
          </div>
